...
namespace atlas {
  namespace detail {
    ...
    class SkipListNode : boost::noncopyable {
      ...
      enum { IS_HEAD_NODE = 1, MARKED_FOR_REMOVAL = (1 << 1), FULLY_LINKED = (1 << 2), };
    ...
    public:
      ...
      static SkipListNode* create(int height, U&& data, bool isHead = false) {
        ...
        SkipListNode* copyHead(SkipListNode* node) {
          std::assert(node != nullptr && height_ > node->height_);
        ...
        std::unique_lock<micro_spin_lock> acquireGuard() {
          return std::unique_lock<micro_spin_lock>(spinLock_);
        }
        ...
        void setIsHeadNode() { setFlags(getFlags() | IS_HEAD_NODE); }
        void setFullyLinked() { setFlags(getFlags() | FULLY_LINKED); }
        void setMarkedForRemoval() { setFlags(getFlags() | MARKED_FOR_REMOVAL); }
      ...
      private:
        ...
        void setFlags(uint16_t flags) { flags_.store(flags, std::memory_order_release); }
    ...
    private:
      ...
      void initLookupTable() {
        ...
        for (int i = 1; i < kMaxHeight - 1; ++i) {
          ...
          sizeLimitTable_[i] = sizeLimit > kMaxSizeLimit ? kMaxSizeLimit : static_cast<size_t>(sizeLimit);
        }
